...
#include "hector_grid_map_compression_rviz_plugin/GridMapVisual.hpp"
#include "hector_grid_map_compression_rviz_plugin/GridMapDisplay.hpp"
#include "hector_grid_map_compression_rviz_plugin/GridMapColorMaps.hpp"

// The following replaces <rviz/frame_manager.h>
#include "hector_grid_map_compression_rviz_plugin/modified/frame_manager.h"
...
CompressedGridMapDisplay::CompressedGridMapDisplay()
{
...
void CompressedGridMapDisplay::onInitialize()
{
...
void CompressedGridMapDisplay::onEnable()
{
  isEnabled_ = true;
  connect(this, &CompressedGridMapDisplay::process, this, &CompressedGridMapDisplay::onProcessMessage);
  MessageFilterDisplay<hector_grid_map_compression_msgs::CompressedGridMap>::onEnable();
}

void CompressedGridMapDisplay::onDisable(){
  MFDClass::onDisable();
  isEnabled_ = false;
}

void CompressedGridMapDisplay::reset()
{
  disconnect(this, &CompressedGridMapDisplay::process, this, &CompressedGridMapDisplay::onProcessMessage);
  MFDClass::reset();
  visuals_.clear();
  if(isEnabled_){
    onEnable();
...
void CompressedGridMapDisplay::updateHistoryLength()
{
  visuals_.rset_capacity(historyLengthProperty_->getInt());
}

void CompressedGridMapDisplay::updateHeightMode()
{
  updateVisualization();
  heightTransformerProperty_->setHidden(heightModeProperty_->getOptionInt() == 1);
}

void CompressedGridMapDisplay::updateColorMode()
{
  updateVisualization();
...
void CompressedGridMapDisplay::updateUseColorMap()
{
  updateVisualization();
  bool useColorMap = useColorMapProperty_->getBool();
...
void CompressedGridMapDisplay::updateGridLines()
{
  updateVisualization();
...
void CompressedGridMapDisplay::updateAutocomputeIntensityBounds()
{
  updateVisualization();
...
void CompressedGridMapDisplay::updateVisualization()
{
...
void CompressedGridMapDisplay::processMessage(const hector_grid_map_compression_msgs::CompressedGridMap::ConstPtr& msg)
{
  grid_map_msgs::GridMap decompressed_msg;
  compression_.decompress(msg, decompressed_msg);
  grid_map_msgs::GridMap::ConstPtr grid_map_msg = boost::make_shared<grid_map_msgs::GridMap>(decompressed_msg);

  process(grid_map_msg);
}

//void CompressedGridMapDisplay::onProcessMessage(const hector_grid_map_compression_msgs::CompressedGridMap::ConstPtr& msg)
void CompressedGridMapDisplay::onProcessMessage(const grid_map_msgs::GridMap::ConstPtr& msg)
{
  ...
  if (!context_->getFrameManager()->getTransform(msg->info.header.frame_id, msg->info.header.stamp,
    ...
    ROS_DEBUG("INFO transforming from frame '%s' to frame '%s'", msg->info.header.frame_id.c_str(),
...
void CompressedGridMapDisplay::updateColorMapList()
{
  updateVisualization();
...
PLUGINLIB_EXPORT_CLASS(hector_grid_map_compression_rviz_plugin::CompressedGridMapDisplay, rviz::Display)